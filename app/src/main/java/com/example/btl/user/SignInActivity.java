...
public class SignInActivity extends AppCompatActivity {
    ...
    private void onClickSignIn() { // set event login
        String email= edtEmail.getText().toString().trim();
        String password= edtPassword.getText().toString().trim();
        FirebaseAuth auth=FirebaseAuth.getInstance();   // kiểm tra bằng firebase
        progressDialog.show();
        auth.signInWithEmailAndPassword(email, password)    // login với email và password
                .addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
                    @Override
                    public void onComplete(@NonNull Task<AuthResult> task) {    // nhận phản hồi
                        if (task.isSuccessful()) {
                            progressDialog.dismiss(); // đóng dialog
